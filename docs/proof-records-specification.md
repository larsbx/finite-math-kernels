...
**The codec is gated at import.** `proof_records/self_test.py` checks four known answers whenever the package loads — the FIPS 180-4 SHA-256 examples first, so a broken primitive is named as one rather than as three changed records, then the committed preimage, identifier and digest of one pinned record — and raises `SelfTestError` rather than returning a wrong identity. Adopted from `larsbx/coop_substrate`, whose application aborts boot when its canonical encoder cannot reproduce its committed vectors: a ledger that content-addresses wrongly should not get as far as being used. This matters here because the package is vendored into repositories that compute identities in CI and never run this suite.

The constants are `proof_records/known_answers.py`, generated by `tools/make_vectors.py` from the same reference ledger as the fixture and committed, so a codec change is a diff somebody accepts rather than a self-test that agrees with itself; `--check` covers both outputs. The four checks call the exported functions -- `preimage_bytes`, `identity`, `digest` -- rather than recomputing them, so a function that alone has moved is caught here and not at the consumer that calls it. The pinned record is chosen by a rule — the lexicographically first ledger entry stored under its own identifier — so it is never one of the deliberately malformed specimens. `self_test.OVERRIDES` is the corruption hook the suite drives to prove the gate can fail; production never writes to it.

The generator is the one importer the gate cannot hold, since it writes the answers: an intended codec change would fail the import before the tool that repairs it could run. `tools/make_vectors.py` sets `PROOF_RECORDS_REGENERATING` around its own import of the reference model and unsets it again, and a package that loads with the gate skipped prints so on stderr. The door opens one way -- absent the variable the gate runs -- and nothing else in the repository sets it.
