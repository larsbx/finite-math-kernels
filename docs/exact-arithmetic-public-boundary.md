...
4. **Division convention.** `bigz_divmod` truncates toward zero: the remainder has the sign of the dividend and `|remainder| < |divisor|`. `bigz_div_exact` rejects unless the remainder is zero. `bigz_gcd` is non-negative and `bigz_gcd(0, 0) = 0`.
5. **Order agrees with value.** `Q.lt` and `Q.le` decide the rational order exactly; `Q.eq` is structural equality of canonical forms and coincides with rational equality.
6. **Encodings are injective and total on accepted values.** `bigz_canonical_bytes` is `Z(sign_code, byte_len, big_endian_magnitude)` with sign codes `0, 1, 2` for zero, positive, negative, an unsigned 64-bit big-endian length, and a magnitude with no leading zero byte. `q_canonical_bytes` is the concatenation of the numerator and denominator encodings. Equal values have equal bytes and distinct values have distinct bytes. Rejected values have no encoding.
