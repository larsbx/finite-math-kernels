...
| Bend 2 challenger | `p < 2^16` (so `p <= 65521`) | Bend 2 has nothing wider than `U32`. `x^2 + c <= p (p - 1) < 2^32` keeps each step exact, and each sum `<= p^2 < 2^32` fits a `U32` field. The program checks the domain itself and answers `unsupported:p`; it cannot see an overflow once it happens, since `U32` arithmetic wraps silently |
...
domain as a measured fact and does not relax the contract to fit it. Reaching
Mojo's `p < 2^24` in Bend would need a `U32` multiply-mod (for example,
double-and-add with conditional subtraction) and two-limb sums (section 10).
...
ranges, `cap = 0`, `cap = p`, `c = 0`, `p = 2`, and the Bend 2 domain edge
`p = 65521`.
...
- **Single-run wall time for `(4093, 1, 4093, 0, 4093)`:** Mojo about 12 ms,
  ...
  Bend 2 binary about 8 ms. A full `p = 65521` block takes Bend 2 about 10 ms
  on 8 threads. These are single runs, not benchmark results, but they show
  the port removed Bend's cost as a reason to shrink the corpus.
...
- Widening the Bend 2 domain to Mojo's `p < 2^24`: a `U32` multiply-mod and
  two-limb sums (section 3.6).
